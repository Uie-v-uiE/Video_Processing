...
屏幕上的画面有**三个来源**，由 PL 里的仲裁自动决定：**网线推流 > SD 卡回放 > 会动的测试图卡**
—— 上电即自动挂载并播放 SD（`AUTOPLAY0/1` 可关），推流一来自动让位、停流后自动接回，
不必拔网线也不必重配 FPGA；`KEY1` 短按仍是旋转 ±1°，**长按 1.2 s** 在
自动 / 锁 ETH / 锁 PS / 锁图卡之间轮转。
交接时序是**量出来的**，不是"看起来会切"：同一块 bit 上七次独立跑，
停流→交回 PS = **210 / 285 / 297 / 357 / 406 / 434 / 481 ms**，推流→接管 = 150–377 ms，七次都是 0 次意外翻转。
...
凭据 `build/frozen_r31_srcmode/arb_handover_green_r31.json`。仲裁看得见的全部输入
