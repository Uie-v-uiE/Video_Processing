...
屏幕上的画面有**三个来源**，由 PL 里的仲裁自动决定：**网线推流 > SD 帧序列（PC 预转换）> 会动的测试图卡**
—— 上电即自动挂载并回放卡上的 **512×300 RGB565 裸帧序列**（`AUTOPLAY0/1` 可关；
不做任何解码，所以这条路的"实时性"是帧库的帧率，不是编解码吞吐），
推流一来自动让位、停流后自动接回，不必拔网线也不必重配 FPGA；`KEY1` 短按仍是旋转 ±1°（**松手才发**），
**长按 0.6 s** 在自动 / ETH / SD / TEST 四态之间轮转（屏上 `Src:` 印的就是后三个词；串口 `src auto|0|1|2` 是同一条出口的另一种写法），LED1 在按住 0.2 s 后亮起表示"正在计时"。
交接时序是**量出来的**，不是"看起来会切"：判据跑过 11 遍，其中**九遍七条全绿**，
停流→交回 PS = **208 / 210 / 285 / 297 / 357 / 394 / 406 / 434 / 481 ms**，推流→接管 43–351 ms、
再推流（可逆性）43–377 ms，九遍都是 0 次意外翻转。
**为什么报区间不报单点**：判据的时间分辨率就是采样周期（100 或 300 ms 两种写法），
设计预算是 200 ms（判「没流」）+ 20 ms（让位静默）≈ 220 ms，再加最多一个采样周期 ⇒ 相容。
另外两遍是**红的**并且保留在同一个文件里：16:54 那一遍在 r32 修复**之前**（V4 根本不交回），
18:11 那一遍是中途的 `PRE=2` 配置（V6 再推流没接管）——它们不是被挑掉的坏数据，是修复过程的凭据。
复跑：`node src/host/arb_handover_test.mjs`（它要按住内核，所以测前自动 `STOP`、测完 `PLAY` 回去）；
凭据 `build/frozen_r32_sdfix/arb_handover_r32b.json`（11 条记录，逐条含 `verdict` 与原始采样）。仲裁看得见的全部输入
（谁拥有屏幕、ETH 是否活着、时基是否可信、它以为的模式、两个引擎的 busy）
都映在 AXI GPIO 的 **lane30** 上，JTAG 读一次就判红绿——不必有人看屏幕。
仍然欠眼睛的是"交回之后画面确实活着"这一条，文档里按"待确认"写。
...
本工程在 Zynq-7020 上实现一条完整的以太网视频处理流水线：上位机以 UDP 推送 512×300 RGB565 画面，PL 侧自研 RGMII/ARP/ICMP/UDP 协议栈与 offset 拼帧，经跨钟 FIFO 与 AXI 打包写入 **DDR 乒乓 bank**，再由提交锁在显示**消隐窗口**内把整帧原子拷进显示 BRAM；HDMI 1024×600 双窗输出——左窗原图，右窗经五级可重构效果链（Gamma 表 / 灰度 / 反色 / 模糊 / 锐化 / Sobel / 二值化 / 腐蚀 / 膨胀）、做连续缩放并可叠加 0–359° 任意角旋转（旋转只作用于右窗，左窗始终是未旋转原画面）。
...
- 效果链：**五级可重构**（级 0 Gamma 查找表 → 级1 灰度/反色 → 级2 模糊/锐化 → 级3 Sobel → 级4 二值化(+判决极性) → 级5 腐蚀/膨胀），
  九种算法各占一位、每级可旁路，串口 `pipe <9 位>` 控制，任意旋转角下均可用
...
### 3. 下载 PS ELF —— 串口命令与 SD 帧序列回放需要（只看右屏缩放可跳过）
...
> 演 SD 帧序列回放这一幕：V7.9#24 起**插着网线也能看到 PS 片源**（PL 里有片源仲裁，ISSUES #49），
