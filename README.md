...
交接时序是**量出来的**，不是"看起来会切"：四次独立跑（同一块 bit）测得
停流→交回 PS 的用时 **210 / 285 / 406 / 434 ms**，推流→接管 150–253 ms，全程 0 次意外翻转。
**报数区间而不是单点**：判据的时间分辨率就是采样周期（100 或 300 ms，取决于采样写法），
而设计预算是 200 ms（判"没流"）+ 20 ms（让位静默等待）≈ 220 ms，实测区间与它相容。
复跑：`node src/host/arb_handover_test.mjs`；凭据 `build/frozen_r31_srcmode/arb_handover_green_r31.json`。仲裁看得见的全部输入
