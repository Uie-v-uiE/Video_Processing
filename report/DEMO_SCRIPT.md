...
| 停流后画面没交回 SD | `node src/host/health_read.mjs` 读 lane30：`owner_eth / eth_live / eth_tb_ok / mode` 四个位就能分清是"仲裁还占着"还是"模式被手动锁在 ETH"；长按 `KEY1` 会把模式挪回 AUTO | `board/README.md` 无人值守交接判据 V1–V6 |
