...
2. 默认是自动呼吸（`inv_scale` 在 256↔512 之间循环，256=原始大小/最大，512=0.5×），
   `ZOOM0` 停、`ZOOM1` 起（V7.7 起这条命令才真的生效）。
   **r53 起可以再补一句"也能钉住"**：串口 `zoom 2` 放大看细节、`zoom 0.25` 收到全图、
   `zoom 0.75` 停在中间档，`zoom auto` 交还呼吸。
   讲这段的顺序建议：先让它呼吸（看得出是连续量），再 `zoom 2` 钉住（看得出是**同一条链**在受控，
   不是两段独立的动画），最后 `zoom auto` —— 交还的那一刻画面**不弹**（从当前倍数继续走）。
   ⚠ 手动档的屏上表现要到 r53 上板后才算演示过（台架 `tb_v94_zoom_sel` 已判过 RTL 侧的八档往返与
   交接不瞬移；"寄存器里的三位真的走到了像素域"这一跳目前只有眼睛能验，见任务 #45）。
3. 讲法：*"缩放与旋转是同一条逆映射链，旋转用 4 个 ROM + 2 个 DSP48 乘加，
   不写浮点、不查表插值，因此每像素周期一次取数就够。倍率是 Q8 的**倒数**尺度，
   所以屏上那八档是查表报出来的 —— 硬件里从头到尾没有过一次除法。"*
...
3. **网线：不再要求"配 bit 之前拔掉"**——PL 里的片源仲裁已经过板级机器判据（#31 位流 + #32 固件，bit `efc89779`）。**可核对的数字**：判据共跑 11 遍，**九遍七条全绿**，停流→交回 PS 的九次实测 **208 / 210 / 285 / 297 / 357 / 394 / 406 / 434 / 481 ms** ⇒ 对外只说 **0.2–0.5 s，并报采样分辨率 300 ms**，不说"立即"；接管 43–351 ms、再推流（可逆）43–377 ms，九遍都是 0 次意外翻转。剩下两遍是红的、也留在同一个文件里（修复前 / 中途配置），念数字时不要说"只跑过七次"。现场要复现就一条命令 `node src/host/arb_handover_test.mjs`（判据窗口 35 s，打印 V1–V6 + V0 七条 PASS/FAIL）。
   ...
   现在的可核对数字（`node src/host/arb_handover_test.mjs` 一条命令现场跑，判据窗口本身 35 s，即凭据里的 t4−t1）：
   推流→接管 **43–351 ms**，停流→交回 PS **208–481 ms**（九遍全绿，逐条数字见上面第 3 点），
   交回后 0 次回跳，再推流 43–377 ms 内可逆。设计预算 200 ms（判"没流"）+ 20 ms（让位静默）≈ 220 ms。
...
| 仲裁交接时序 | 接管 43–351 ms、**停流后 208–481 ms 交回**、0 次回跳、可逆（九遍全绿／11 遍含 2 遍红；预算 220 ms；分辨率=采样周期） | `build/frozen_r32_sdfix/arb_handover_r32b.json` |
| SD 帧序列（预转换）回放速率 | 4398 帧 / 9 文件，**29.999 fps** 连续 46 个 100 帧窗口（≈9.0 MB/s 卡读带宽）；上电自动开播，串口自己报速率 | `board/uart_r28_autoplay.txt`、`board/uart_50_probe.txt` |
