...
| 片源仲裁交接（2026-09-24，同一块 bit 四次独立跑） | 停流→交回 PS：**210 / 285 / 406 / 434 ms**；推流→接管 150–253 ms；再推流可逆；全程 0 次意外翻转。设计预算 220 ms（200 判死 + 20 静默），实测区间与之相容；**报数区间的理由是判据的时间分辨率=采样周期**（100 或 300 ms 两种写法）。凭据 `build/frozen_r31_srcmode/arb_handover_green_r31.json`（100 ms 那一次）；**屏幕观感仍待眼睛** |
| 效果/旋转/选源/推流 | 与缩放并存可用 |
