...
### 4.2 规律黑点 = 分包 1396（写选通覆盖），v6.4 已在硬件里免疫

同板同 bit 同会话，`--test frameid` 200 帧 @15fps，回读两个 bank：

| 分包 | 最新帧命中率 | 每帧空洞（16bit 字） | 空洞内容 |
|---|---|---|---|
| 1396 B（本机多份历史 `video_sender.py` 的取值） | 99.9% | **222（111 处 × 2）** | 0x0000 ⇒ 均匀散布的黑点 |
...
| 指标 | v6.3 + 1396 | **v6.4 + 1396** |
|---|---|---|
| 最新帧 16bit 命中率 | 99.9% | **100.0%** |
| 每帧空洞（16bit 字） | 222（111 处） | **0** |
| 包内六个字节带丢字率 | 0.1%~0.2% | **全 0.0%** |
| 帧号跨度 | 相邻两帧 | **每 bank 恰好一帧**（#28 / #29），且这次连 §4.1 的帧尾残留都没出现 |

1392 工况无需重测：对齐流量下 `cur_keep=4'b1111` ⇒ `WSTRB=0xFF`，与 v6.3 逐位等价，
且 `tb_v6_ingress_integrity` 的 PART / +FULL（1392）两个模式在 v6.4 下仍 PASS。
