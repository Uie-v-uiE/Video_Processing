...
| `build/frozen_r25_arbfix/`（`ad4aa31c`）= 当时的候选默认（06:4x 起被 #31/#32 取代，见本表末行） | 仲裁修正版（判据要与时基健康相与，见 `ISSUES.md` #49）；**两条眼睛判据未过之前不改默认** | `board/README.md` "需要肉眼确认的项" 第 7/8/9 行（06:4x 起多了 KEY1 长按那一条） |
