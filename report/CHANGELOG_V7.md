...
`（09-24 起本机已无 ModelSim，这条不可复跑；可复现路径只有 xsim）`
- ModelSim AE 2020.1（历史工具，本机已卸载）：`vlog` 当时可用（**它抓到过一处 Vivado 侧不易读的错** —— 标识符先出现在
...
## V7.10（R32 → R42，2026-09-23 深夜 → 09-24 06:3x）—— 第三源、可机器判定的交接，以及一条判据把自己人判红两次
...
L1 全量 **47/47**（`sim/results/regression_v79_r39.txt`；06:3x 又用当前这棵树重跑一遍 = `regression_v79_r42.txt`，同样 47/47）。
中间版 #29 曾到 WNS +1.001，最终版为可观测口付了 0.24 ns —— 两个数都记，不挑好看的念。

**05:3x–06:3x 又推进的（这一段把两条"待办"变成"已结"）**：
① **#50 结案**：根因是 PS 固件 `dir_lookup` 把 FAT32 目录项的高簇字按大端拼（首簇 ≥ 65536，
   也就是文件起点落在数据区 1 GiB 之后才发作 ⇒ 只有第 8 个文件中招），**卡与 FAT 都是好的**；
   修完整卡 4398 帧第一次播完并自动回绕，并发 165 s 跨点零失败。凭据/判据/反例冻进
   `build/frozen_r32_sdfix/`（elf `c00b6553`，bit 仍是 `efc89779`）。原先那两条出路（重拷卡 /
   删第 8 个文件）**作废**。
② **§10 U14 结案**：`eth_rxc→clk_fpga_0` 从 Critical 掉到 Warning 不是报告抖动 —— #24 删掉了
   axi 域里那级 `3FF(eth_link)`，该行唯一的 Unsafe 端点 1→0、端点 272→271 ⇒ 可以记成改进。
③ 技能包 16 → **17** 项（新增 `failing_read_prints_geometry.md`：定点失败要让失败行自己报几何量）。
④ 交接判据在 #32 的 elf 上**连跑两遍全绿**（交回 297 / 357 ms；累计七次实测 210–481 ms）；
   L1 台架用当前这棵树重跑 **47/47**（`sim/results/regression_v79_r42.txt`，不再是引用 r39）。

**仍然没做/没解的**：链路内时延**未测**（协议无时间戳、PL 里也没有打点寄存器，
不写没有凭据的数字）；三条眼睛判据待用户确认（停流交回后画面真的是 SD 在动 /
并发不闪不抢 / KEY1 长按四态轮转且图卡会动）；KU5P 与 MIPI 仍按用户决定停着；
双线性插值仍停在 tag `v7.8-bilinear-wip`，没合进主线。
