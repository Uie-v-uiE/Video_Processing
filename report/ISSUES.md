...
### 49. 片源仲裁：`link_active` 是"曾经收过包"，两个搬运机又没有互锁（改完待门禁 + 板级）
...
| 层 | 谁负责 | 判断 |
|---|---|---|
| "活着没" | `link_monitor.lm_live`（eth_rxc 域） | `have_base && stall_ms < LIVE_MS(200 ms)`；与快照里 flag5.bit3 同一判据，只多 `have_base`（否则复位后 `stall_ms=0` 会假装活着）。快照那一位**没动** ⇒ PS 读回来的语义不变 |
...
| "谁在显示" | `pl_video_top` | `bram_or_hold` 保持 #47 的 `eth_link_pix \| ps_src_seen`（**不动已验过的通路**）；`pub_consume` 从 `!eth_link_pix` 换成 `!eth_live_pix` |

`eth_link` 一路原样保留（OSD/状态与 R08~R10 三条板级结论都挂在它上面），新增的 `eth_live`
是第二条独立电平；跨域仍是 eth_rxc→axi_clk 与 eth_rxc→clk_pix 那两对既有配对 ⇒ `cdc.rpt` 的
行数不该变（门禁里那一栏就是这么判的）。`u_aw.frame_busy` 从悬空改成仲裁输入。

**台架**（先于构建跑过）：
- `sim/tb_v796_src_arb.v` 16 条断言全 PASS，含三条反面对照：① 拉起 `fill_busy`/`row_busy`
  时 owner **必须不动**（600 拍远超 200 拍的等待窗）；② 同一段激励下 `T_OFF_CYC=0` 的对照实例
  确实在翻（≥3 次），而被测实例在抖动里 ≤1 次 ⇒ "不抖"不是"翻不动"；③ 用**默认参数**再例化一个，
  断言 100k 拍（= 20 ms）之后还没让位 ⇒ 硬件里那个常数被人改小会被这台架抓住，而不是只在文档里。
- `sim/tb_link_monitor.v` 另开一个 `lm_live` 段（第二个实例、自己的激励，不动原时间线）：
  复位后必须 0、`frame_done` 立刻 1、190 拍仍 1、过 200 ms 落 0、再来一帧回升。
- 全量 L1 **44/44 PASS**（新加的那台架是第 44 个）。

**待**：构建 #24 的七项门禁；板级一次跑完"插着线推流 → 停流 ~0.2 s 后 SD 自动接上 → 重新推流自动切回"，
外加之前欠着的 `ZOOM0/1`（SD 画面上）。
