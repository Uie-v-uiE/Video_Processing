...
0. **（先做这一步）SD 卡要被电重启一次。** 我 02:4x 之后 `PLAY` 报 `SD read failed`、
   `STAT` 报 `sd=0` —— 卡的控制器被我"整轮按住 A9"那种测量弄僵了
   （`XSdPs_CfgInitialize` 每个上电周期只成功一次，ISSUES #45/#50）。**拔掉 12 V 再插回去**就恢复。
   规矩已经写进 `board/README.md`（测之前 `STOP`、测完 `PLAY`），`arb_handover_test.mjs` 也自动做了。
1. **（5 分钟，需要你眼睛）#31 上屏确认 —— 只欠这三条，机器那一半已经过了**
   ...
   `owner_eth` 在 0.2–0.45 s 内交回（四次独立跑，含采样分辨率），但"交回之后画面活了"仍然只能看；
