...
| **R24** build#19（#18 + `eth_ctrl` 发送仲裁修复 = ISSUES #37） | +0.598 | +0.043 | 0/23424 | 64.64%（90.5 tile，未增） | 21.89%（2911） | 13.50%（7185） | 5.45%（5802） | 2.184 W(Dynamic) / 2.358 W(total)（与 #18 相同） | 0（12503 根全布通，0 布线错误） | 0 新行（`sys_clk↔eth_rxc` 端点 1859→1861，与"多一级寄存"一致） | **PASS ⇒ 明早默认下这一块**（md5 `545a27a1`，**成套含 bit** 冻结 `build/frozen_r19_arb/`；WNS 比 #18 低 0.4 ns 与这笔无关 —— 两次最差路径同为 `x_d_reg→u_osd/b_reg`，见 §15 R24-C） |
...
Slice LUT 7185(13.50%)、Slice 2911(21.89%)、Reg 5802(5.45%) / Dynamic 2.184 W / 12503 根全布通 0 错误 /
