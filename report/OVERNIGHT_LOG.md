...
## §40 功耗这一维（2026-09-25 06:58）：2.182 W 这个数**不能当成绩念**，而且大头不在我们写的逻辑里

读 `build/frozen_r56_cdcfanout/power.rpt`（就是 r56 那一版的 `report_power`），两件事必须先说清楚：

**① 工具自己给的置信度是 `Low`**（报告 §1.3）：`Clock nodes activity = High`（>95 % 时钟指定过），但
`I/O nodes activity = Low`（>75 % 的输入没有用户给的翻转率）、`Internal nodes activity = Medium`
（内部节点指定不足 25 %）⇒ **总体 Low**。所以 2.182 W 是"时钟结构可信、活动量按默认假设"的估算，
**对外不许写成"实测功耗 2.18 W"**。要让它变准只有两条：喂真实活动量（仿真 VCD/SAIF，或按量到的
30 fps / 像素率手工 `set_switching_activity`），或者拿功率计量整板（**另一个口径**：含 PS、DDR、HDMI 芯片）。
在做到其中一条之前，正确的说法是"工具按默认翻转率估 2.18 W（总体置信度 Low）"。

**② 就算这个数可信，能动的地方也不在自研逻辑**（同一报告 §3.1 By Hierarchy）：

| 块 | 估算 (W) | 占 2.182 的比例 | 是什么 |
|---|---|---|---|
| `u_bd/…/processing_system7_0` | **1.534** | **70 %** | **PS 硬核**（ARM + DDR 控制器…），不是我们写的 RTL，数值主要来自默认假设 |
| `u_pl`（整个 PL 视频侧） | 0.458 | 21 % | 其中 `u_clk` 0.107 + `u_dvi` 0.134（四个 ODDR）+ `u_fb` **0.163**（帧缓存 BRAM）= 0.404，已占 `u_pl` 的 88 % |
| ├ 处理链 `u_pipe`（blur/sobel/morph/sharpen/gamma…） | **0.018** | 0.8 % | ⇒ 在这条链上"省功耗"最多省百分之零点几 |
| `u_eth`（自研 RGMII/ARP/ICMP/UDP 全栈） | **0.051** | 2.3 % | 五个子模块加起来 |
| `u_idelay_clkgen` | 0.106 | 5 % | 给 IDELAY 用的 MMCM |

⇒ **步骤② 的功耗结论已经成形，而且是"别动手"型的**：自研逻辑里唯一有量级的两项是
帧缓存 BRAM（0.163 W）和像素/串行化时钟（≈0.24 W）。前者要动就得动 UltraRAM 那条**没收口**的路
（`CONTEST_CHECKLIST` 第 3 项：2025.2.1 把 `ram_style="ultramark"` 与 `"ultraram"` 两种拼法都拒了、
静默退回 `auto`，量到的还是 72 tile BRAM / 0 URAM）。**那条账里有一处我当时没核就写下的东西**：
清单写"直接例化 `URAM1240`" —— 而 `URAM1240` 是 **UltraScale+** 的原语，**7 系的 UltraRAM 是 `URAM288`/`URAM288E`**；
所以"实验失败"里有多少属于"工具真不为这个形状选 UltraRAM"、有多少属于"我拿错了器件族的名字"，
**今天还不知道** ⇒ 深度这一轮值得重开一次小实验（先查 7 系 UG901 的 `ram_style` 合法值与 URAM288 的端口形状，
再决定要不要例化）。后者不能降（面板就是要那个像素时钟）。
**把 PS 的 1.534 W 算进"作品的功耗"是不诚实的**：那是 SoC 里那颗硬块在默认假设下的估值，
作品跑不跑它都在。对外如果要给数字，给"PL 侧自研逻辑估算 ≈0.5 W / 整器件估 2.18 W（工具置信度 Low）、未做实测"。

（同时正在跑 **build #38 = 与 r56 逐字节相同的 RTL + `Performance_ExploreWithRemap`**，
日志 `build/r57_build38.log`；构建脚本现在会把策略名打进日志（`BUILD_STRATEGY …`），
所以这颗 bit 自带出身。这一版同时补两件事：功耗那一列的真数、以及"两次独立构建同方向"的第一次。）

## §41 步骤② 第一次真正的"采纳"：把实现策略定成 `Performance_ExploreWithRemap`（r57），以及一次没有答上来的实验（UltraRAM）

### 1）采纳判据三条全凑齐了（这是第一次"数字变好"可以记账）

| 版本 | 策略 | bit md5 | WNS | WHS | 失败端点 | Dynamic | 门禁 | 板级 |
|---|---|---|---|---|---|---|---|---|
| r56（#37） | 工具默认 | `c15454ae` | 0.540 | 0.019 | 0 / 0 | 2.182 W | 14/14 | 八条 + 51/51 |
| **r57（#38）** | `ExploreWithRemap` | **`1d588efc`** | 0.518 | **0.028** | 0 / 0 | 2.185 W | 14/14 | **八条 + 51/51**（接管 417 ms、交回 471 ms、再推 96 ms、稳占段 28/28、原因位不可用 0、`drop_words=0`、`osd_ms_matches_tot=true`、SD 29.999 fps） |
| r57b（#39，独立复跑） | 同上 | `06e56edc` | 0.518 | **0.028** | 0 / 0 | 2.185 W | 14/14 | 未刷（与 #38 同策略同 RTL，见下面的"为什么可以不再刷"） |

三条判据各自的凭据：① 完整构建过 14/14 —— `build/gates_r57_build38.txt` 与 `build/gates_r57b.txt`；
② 板级全绿 —— `build/frozen_r57_remap/r57_board_verify_console.txt`（冻结件里那三件套 md5 就是 #38 的）；
③ 两次独立构建同方向 —— #38 与 #39 的 bit **逐字节不同**（`1d588efc` vs `06e56edc`）却给出**同样的三个数**，
所以这不是某一次布局的运气。策略名写在两份构建日志里（`BUILD_STRATEGY Performance_ExploreWithRemap`），
产物自带出身，不靠人记住当时点了什么。

**为什么 #39 可以不再刷板**：#39 相对 #38 没有改任何输入（同一份 RTL、同一策略、同一约束），
它的作用只是"第二次实现"来证伪"运气说"；要交付的那一颗是 **#38**（已刷已验）。
如果哪天 #39 这类复跑变成了要交付的件，就得重走板级 —— 这条界限写在这里，免得以后含糊。

**对外口径的升级**（这是本晚唯一一笔"从不能说到能说"的变化）：
> 最小 hold 裕量从 **0.019 ns** 提升到 **0.028 ns**，由实现策略给出，两次独立构建一致；
> 代价是 setup 裕量 0.540 → 0.518 ns、估算功耗 +0.14 %（2.182 → 2.185 W，仍是工具低置信度估算）。

仍然**不许**说成"我们把时序修好了"：hold 在 r56 就是 met 的（失败端点 0），这是余量的移动，不是违例的修复；
而且路径级归因（§39）说明这一族余量由偏斜/布线决定，**RTL 侧无事可做**。

### 2）一次没答上来的实验：这颗 7020 到底有没有 UltraRAM 站点（三次问法全被自己的对照证伪）

为了把 `CONTEST_CHECKLIST` 第 3 项那句"9 块 UltraRAM"从"算出来的"变成"有的"，
写了两个探针（`build/tcl/uram_probe.tcl`、`build/tcl/uram_presence.tcl` + `uram_sites.tcl`）。
**答上来的只有一半，另一半是我自己的方法不对**：

| 问法 | 结果 | 能不能用 |
|---|---|---|
| 综合真实形状：300 KB 的 1 写 1 读 RAM | **96 个 RAMB36、0 个 URAM**；加 `ram_style="ultramark"` 被工具明说无效（`WARNING [Synth 8-11376] … 'ultramark' is not a valid value. The default value 'auto' will be used`） | ✅ 事实，可引用 |
| 同样容量改成 **1 写 4 读** | **384 个 RAMB36（正好 4 倍）、0 URAM** | ✅ 与 §2（双线性要第二个读口时量到 80→160）互相独立地印证了"**读口数量直接乘 BRAM**"这条设计约束 |
| `get_lib_cells` 查库里有没有 `URAM288` | 返回 0 —— **但阳性对照 `RAMB36E1`/`FDRE` 也返回 0** | ❌ 问法是坏的，0 不算数 |
| 直接例化原语 | `RAMB36E1` 通过、**`URAM288` 通过**、`URAM288E` 失败、`URAM1240` 失败（对照方向正确） | ⚠ 只证明"库里有这个原语"，不证明"这颗器件有站点" |
| `get_sites -filter {TYPE == …}` 数站点 | `RAMB36`/`DSP48E1`/**连 `IOB` 都返回 0** | ❌ 又是坏问法（第三次尝试，按规矩停手） |

⇒ **结论要写成两句不同的话**：
① "**工具不会为帧缓存这种形状选 UltraRAM，且 `ram_style` 在这版里没有 UltraRAM 相关的合法取值**"——
这条现在有凭据（`build/uram_probe_console.txt` 里那条 `Synth 8-11376`）；
② "**这颗器件有没有 URAM 站点**"——**本期没有答上来**，三次 TCL 问法都被自己的阳性对照打掉，
正确下一步是去查器件手册（DS187 / PG091 的 Zynq-7020 资源表）而不是继续猜命令语法。
`CONTEST_CHECKLIST` 第 3 项已按这两句改写，"9 块 UltraRAM"降级为**未证实且方向可能根本不成立**。

一次实验的元教训（进 `skill/bench_self_inflicted_reds.md` 的家族）：
**"查不到"和"不存在"是两件事**。探针必须在同一个环境里带一个明知存在的东西做阳性对照
（这次是 `RAMB36E1` / `IOB`），否则一个坏命令返回的 0 会被当成"器件没有"这种硬结论 ——
那是我这类工具最容易造的假事实。

换了条路也想把它答上，两条都没成（记下来，免得下次再花同样的时间）：
① 网上取 DS190/DS187 的表 —— PDF 是可读的但**表格在压缩流里**，抓取工具拿不到具体格子的数；
② 查本机 Vivado 的 parts 数据库（`data/parts/xilinx/zynq/devint/*.xml`）—— 前几个文件里根本没有
   `URAM` 字样（那些文件是 IOB/时钟/配置模式，不是资源计数表）。
⇒ 按"同一问题三次不通就停手"的规矩**收工**，这一条保持"未证实、本期不提 UltraRAM"的口径。
真要答它，正确的一步是把 DS187/DS190 的 PDF 下到本地再用 `pdftotext` 读那张表。


### 3）现场状态

- 板上跑的 = **r57（#38，bit `1d588efc`）**；`build/` 里的活产物（bit/xsa/全套报告）已从
  `frozen_r57_remap/` 拷回，**与板上那颗逐字节一致**（#39 的实现产物只留在 `.runs`，不当交付件）。
- 演示默认仍是 #23（`frozen_r23_srcseen`）——换默认是用户的决定，不因为我多验了一版就自动改。
- 探针的工程目录已删（`build/uram_probe/` 只留 8 个 .v/.rpt/日志，21 KB），凭据是
  `build/uram_probe_console.txt`、`build/uram_presence_console{,2,3}.txt`、`build/uram_sites_console.txt`。
...
## §42 r58（2026-09-25 07:20–11:55）：用户坐在屏幕前的两轮报告 —— 词表、命令进得去的路、以及一次门禁抓我自己

### 1）这一轮的输入不是我的计划，是用户的眼睛

上午他连着报了两批。第一批的原文很长，摘几句关键的：
"SRC 写的有点像 Sro"、"card 对应的是图卡、eth 对应的是 sd 卡"、"gamma 哪一行跑到分界线去了"、
"缩放的时候线会移位，并且碰到分割线的时候会在分割线周围出现类似的颜色条"、
"串口发命令为什么要发 8 位或者十位 pipe 00001100 类似这种才能读到，五位读不到"、
"pipe 我传了几个命令现在变成了 00102，怎么 2 都来了"。
第二批是他坐在板子前实测之后："ps 和 card 是反的 … sd 卡上面显示的是 ps … 长按要按几次 …
现在切换画面很顺畅 … 延迟这一项也可以正常显示了 … 开始修复推进"。

**这批报告里有一条我先判错了并当场更正过**（记在这里，免得以后有人以为它是硬件问题）：
我第一反应是"仲裁编码契约错了"（`src_arb` 注释写 `10=锁PS` 而 `pl_video_top` 的 `M_PS=3`），
但 `pl_video_top.v:174` 有一行适配器把环上的 3 翻译成仲裁要的 2 ⇒ 契约没错。
真相是板子被长按停在"锁 SD"，而锁 SD 时 `owner_eth` 本来就不该动 ——
**红的是我的脚本没有起点**，不是仲裁坏了。修法是 `arb_handover_test.mjs` 开跑前先 `src auto`
并**验回显 `mode=0`**（读到旧固件没有 `mode=` 字段就明说"起点不可知"）。
⇒ 可迁移的那条进了 `skill/`：**凡是"读一个状态机并判断它是否响应"的判据，必须有一步把状态机放回已知态，
并且要验那一步真的生效**，不是发一条命令就算。

### 2）五条修完，各自的凭据

| 用户看到的 | 机器侧改了什么 | 凭据 |
|---|---|---|
| 命令救不出被钉住的画面 | GPIO_0 `[24:23]` = 模式码、`[22]` = 翻转位；`src_mode` 里翻转位走 3 级链、**码走一条等长延迟线**，沿到链尾那一刻取延迟线尾部 ⇒ PS 那两笔写隔多久都不影响正确性。覆盖期间长按 = **只交还控制权不另走一步**；`src auto` 把环也清回 AUTO | `tb_v82_src_mode` T8–T12（含"沿之后才改码"的顺序反例）；板级 `src auto/0/2/1/auto` 五条电池 + 仲裁八条全绿（接管 371 ms、交回 216 ms） |
| "ps 和 card 是反的"（说了三轮） | 词表一次定稿 **`ETH` / `SD` / `TEST`**，屏上/串口/固件档名/两个 host 脚本/台架金表一起改，**编号不动**（AUTO=0、ETH=1、TEST=2、SD=3，格雷码保住） | `tb_osd_lines` T3；`health_read --selfcheck` 28/28；板上 `lane30 … "mode":"TEST"` |
| "gamma 那行跑到分界线" | 字格 18 px、起点 16 px ⇒ 28 格 = 520 px > 分割线 511；第二行两个分隔收成单空格（26 格） | `tb_osd_lines` T13 逐行量 `X0+k*CW ≤ 511`。**顺带抓到一条我原本没看见的**：`(Auto)` 一旦真亮起来第四行 = 31 格 = 574 px ⇒ 约束写进 T13 注释，V8-4b 动那一格之前必须先收第四行 |
| "`Src` 像 `Sro`、`Pipe` 的 p 和 e 一样高" | 字模 `c` 右列多点了两个点（就成了 o）、`p` 左列在第二行断开 | T14 逐像素对**台架里独立手写**的点阵（不从 RTL 抄 —— 同源就永远抓不到） |
| "为什么发 8 位才读得到""怎么 2 都来了" | 两条不同性质：屏上那五格每格 0..3 是"这一级选了第几个算法"（**不是 bug，是口径**，`osd_overlay.v:114-125`）；真 bug 是 `parse_bits` 只挡第 10 个字符 ⇒ 6/7/8 位被**静默补零**当成 9 位。现在长度只收 5 或 9，**拒的那条路径一个寄存器都不写**，并新增 `pipe show` 按名字念 | `src/host/pipe_len_check.mjs`：位号五份一致（RTL 位定义表 + RTL 老五位翻译 ↔ `SEL_*` 宏 ↔ `SEL_NAME` 下标 ↔ help 两句位序）+ 16 条用例（函数原文抠出来用宿主 gcc 现编）+ 两条变异对照（删长度口径红 5/6、删位序契约红 3/5）；板级三条新电池行含 `!sel=` 反例 |

电池从 51 条到 **59 条**，`--align` 先证明判据表与命令逐行对位（错位会让整轮误诊成"固件坏了"）。

全量 L1 这一版收尾时是 **62 份 62/0**（61 份旧账 + 新增的 `tb_v99_unisim_sim`，见 `build/frozen_r58_ethsdtest/l1_r58_62of62_console.txt` 末行 `SIM DONE pass=62 fail=0`）。

### 3）这一轮唯一一次"红是真的"：门禁第 6 项抓到我自己上一笔

build #40 门禁红：`clkout0_1>clk_fpga_0` 新增 27 端点 / 2 unsafe。原因就是我为了让串口能钉模式，
把 `mode` 写成组合选择 ⇒ 同步器前面挂了一级 LUT。数字与 #34 那次**一模一样**，
这说明"红了先怀疑自己上一笔"这条流程有效 —— 它给出的是同一个形状的第二份证据，不是工具抖动。
修法：`mode_q` 触发器、**唯一驱动**、单独一个 always 块（两个块都赋它就是 #61 那个多驱动 net）。
#41、#42 都因此回到 14/14 绿。顺手把门禁自己的反例测试重跑：`gates_cdc_test.sh` 5/5 PASS。

**采纳的是 #42**（bit `e88f6431` / xsa `8410e0a9` / elf `9144bb40`，WNS 0.321、WHS 0.028，
BRAM 92/140 = 65.71 %、LUT 11126、Dynamic 2.185 W，`check_ports` 比对点 546→548 violations=0），
已刷板、机器复验全绿、冻结在 `build/frozen_r58_ethsdtest/`（MANIFEST 里有复现命令与"还欠什么"）。

### 4）一件副产品：为什么"顶层没有台架"这件事以前无解，今天有解了

`pl_video_top` 例化了 MMCM/BUFG/OBUFDS/OSERDESE2，而本机 xsim **没有 UNISIM 库**
（`Module <MMCME2_BASE> not found`）⇒ 顶层台架不是"没写"，是**起不来**。
今天补了 `sim/prim/` 两个占位件（分频比全从例化参数读回，模型里一个数都不写）+
`tb_v99_unisim_sim` 给占位件自己立判据（比例 5×/4× 才是有牙的那条，绝对值 50/250/200 是第二条；
外加 C4"窗口太短必须报量不出"防"空跑判据"）。第一版用等事件量周期**挂死**过
——挂死的台架比红的更糟（全量回归是顺序跑的），所以最终版只有固定时间窗 + 计数器 + 看门狗。
⇒ 任务 #49（V8-4b 的顶层台架）从"可能做不到"变成"可以开工"。

### 5）现在的账：功能轮剩下的两块都已经被用户批准

`ISSUES #62` 里写完了 4b 的六步实施方案与风险排序 —— 关键结论是**它不需要第二个读口**：
逐像素混合要的是同一条地址流上"链子前 / 链子后"两个抽头，代价是一条 4 行的原图延迟线（≈5 块 RAMB36）
与行缓存翻倍（≈5~6 块），总占用 92→约 103/140。第二步是双线性（`v7.8-bilinear-wip`），
它三次红过时序这件事仍然要在采纳前当面重验，不许因为"用户说都做"就放松门禁。

---

## §43 r59a（2026-09-25 11:30–13:50）：顶层台架第一次跑通，顺手挖出缝旁边那条暗带的真因

### 1）为什么这一轮从"台架"开始而不是从 RTL 开始

上午用户一句话给了两件东西：词表定稿（`ETH / SD / TEST`，r58 已上板，见 §42 之后那轮）和
"把整体的项目功能都做"。剩下最大的一块是 V8-4b（缝 0~100 % 可动 + 逐像素原图/处理图混合），
而 `ISSUES #62` 的风险②写着：**"没有任何台架例化 `pl_video_top`"** ⇒ 混色级那一拍的坐标标签
差一列就是一条可见竖带，而今天没有任何东西能看见它。

于是先做台架。第一个发现是**为什么以前没人写**：本机 xsim 没有 UNISIM 库
（`xelab` 报 `Module <MMCME2_BASE> not found`）⇒ 举要 `clk_gen` 的顶层在仿真里根本起不来。
补了 `sim/prim/MMCME2_BASE.v` + `sim/prim/unisims_sim.v`（**分频比全部从例化参数读回来，模型里一个数都不写**），
并把它们同时加进 `run_sim.tcl` 与 `run_one.sh` 两处清单（只加一处 = 2026-09-23 那次的 ELAB_FAIL）。
占位件自己也要被判：`tb_v99_unisim_sim` 量出 1 µs 窗口里 pix/5x/200m = 50/250/200 拍
⇒ 50/250/200 MHz 精确成立，且判的是**比例**（`CLKOUT1_DIVIDE` 从 4 改 5 会红），不是绝对值。

### 2）`tb_v98_top_seam`：喂一张"像素值 = 自己坐标"的图，标签与内容一减就是差

DDR 里那帧合成图的每个像素是 `{row[7:0], col[7:0]}`，屏上任一格的"我以为是哪一格"与"是哪一格"能直接对。
跑通之后第一眼就看到（`build/tb_v98_run.log`）：

```
DBG 左窗 x=100 y=300 | 顶层源坐标 sx_l=109 sy_l=150 oob=0 | rd_addr=76908
```

`sy_l=150 = y>>1` ⇒ 行对；`mix_x=100` 那一刻地址已经在 `sx_l=109` ⇒ **列标签比内容旧 9 列**。
算术也对得上：内容站在 `3(cx/cy 打拍) + 1(rd_addr_q) + 1(BRAM 读出) + 15(u_pipe.LATENCY) = 20` 级，
而 `split_display` 拿的是 `x_d[11]` ⇒ 20 − 11 = 9 ✓。

**后果**：`pix_left/pix_right` 是按内容互相强制清零的（`pl_video_top.v:711-714` 用延迟过的 `left_pix`），
而 `split_display` 按标签二选一 ⇒ 缝左侧那 9 列里标签说"左窗"、内容其实是右窗那一路（=0）
⇒ 选中 0 ⇒ 一条约 9 像素宽的近黑竖带。这就是用户上午念的
"缩放碰到分割线的时候会在分割线周围出现类似的颜色条"的**第二个成分**
（第一个是故意画的 2 px 蓝线，`#56-2(a)`；行缓存漏上一行末尾是第一个成分之外的第三种，已由 #54 收掉）。
⇒ 记成 **ISSUES #68**，修法记成 **r59a**（本轮）。

### 3）r59a 刻意只做"标签"这一半

* `split_display` 多两个入口：`x_sel`（**与两个像素抽头同一级**的列坐标，只管"这一格选哪一路"与"标记线画在哪一列"）
  和 `marker`（那条 2 px 蓝线的开关，默认仍 1 = 画）。
  `x/y/de/hs/vs` 继续按原级数穿过输出寄存器 ⇒ **OSD 一个像素都不移**（它的坐标仍是 `x_d11`）。
* 顶层把级数写成算式：`localparam MIX_D = 3 + 1 + 1 + u_pipe.LATENCY;`，`SB` 由它撑到 21 级
  ⇒ 全设计只有一份"混色级在第几级"的说法，而且是**推出来的**，不再抄字面量 11。
* 判据：`tb_v97_seam_scan` 新增 **S7a**（`marker=0` ⇒ 一列蓝都不许有）、**S7b**（关线不许顺手改选择逻辑）、
  **S7c**（故意让 `x_sel` 比 `x` 落后 9 列 ⇒ 蓝线必须落在 520/521 而不是 511/512）、
  **S8**（左右内容的分界同样跟着 `x_sel`）；`tb_v100_raw_delay` 验 4 行环形缓存。
* 新增的 `raw_line_delay` **第一版是错的**：用 4 个 `line_cache` 串起来 ⇒ 传递函数是
  `(行 −4, 列 −4)`（台架量出来的，见 `build/tb_v100_console.txt`）。串级每级多花一拍 ⇒ 列方向累计偏移。
  改成**一块 1W1R 的行环形 RAM**（写槽 = y 低位、读槽 = (y−LINES) 同一几位，`mod` 就是切位 ⇒ 无除法，#58）
  ⇒ 延迟恰好 LINES 行 + 1 拍、列不偏；顺带比级联省一半 BRAM（约 4 块 vs 8 块）。

### 4）这一轮我自己造的两次"假红"（都写进 #68，因为它们比修好的东西更值得记住）

1. 台架的 AXI 从机写成 `arready = !busy`，而 `axi_frame_writer64.v:88` 是**重叠发下一个 AR** 的
   ⇒ 整轮只搬完一帧，其余格子在仿真里是 X、屏上落回测试图卡。差点把"从机不会接单"报成"顶层内容错了"。
2. 修完之后我把 C-tap 写成"当拍的 `sx_l − 1` 必须等于当拍的 `x_sel`" —— 这两个信号本来描述
   **相差 17 拍的两个像素**，所以那一轮"1 200 000 格全部偏差、首格 8"红的是我的算式。
   ⇒ 该条已在台架里**降级为只报数**（`OBS C-tap`），并写明"能判'标签与内容同列'的只有两条路"：
   ① 内容判据（要先在仿真里把 DDR→帧缓存的搬运建模对 —— 那仍是本轮没做完的缺口，台架里标了 `NOTE C1/C2`）；
   ② **板上看缝左右 10 列有没有暗带**（`board/README.md` 第 12 行与新增第 22 行，r59a 之后必看）。
   教训（"同一拍取两个不同流水线级数的信号互相比，几乎必然错"）已进 `skill/bench_self_inflicted_reds.md`。

### 4bis）13:20 探针回来：①那条路的"缺口"其实和我指认的位置不一样（同日第三次尺子错）

为了让"内容判据到底能不能做"这件事**有数可查**，我给台架加了三个计数器（帧缓存写脉冲、
`copy_hold` 挂着几拍、搬运机跑到第几行）。结果把它原先的解释**证伪**了：

```
PROBE fb_wr_pulses=115200 (一帧要 38400)  copy_hold 拍了 0  u_aw.active=122402 最后到的 row=0
DIAG fb_vis=1 ... tap_raw 全黑比例 19200/1200000
```

`115200 = 3 × 38400` ⇒ **拷贝路径在仿真里是好的**，三帧完整落进显示帧缓存，全黑格只占 1.6 %，
`dbg_src=0308` 解出来是一个真坐标。所以"屏上大片 X"不是"没搬进来"。真正的缺口在**我这把尺子**：
`bad_l = 459452`（38 %）与同一份直方图的"Δcol 落在 ±5 之外 1 182 618 点"**互相打脸**
（后者按定义要求前者至少 118 万）⇒ 左窗"这一格本该是源图哪一格"的映射式子仍然写错，
在这种状态下判内容对齐，红的只会是我的算式（同族第三次）。
`tb_v98_top_seam.v` 里那段 `NOTE C1/C2` 已按探针结果**改写**（保留原解释如何被证伪，
免得以后有人以为那是硬件问题），并把 C1/C2 的转换条件写明：先让那两个自相矛盾的数一致。

同一批里另外两件小事都影响"别人能不能复现我的凭据"：

1. **看门狗把构建日志里回显的脚本源码当成了执行输出**（详见 `skill/bench_self_inflicted_reds.md` 第九签名）：
   `vivado -mode batch` 会把每一行 tcl 前缀 `# ` 再打一遍，所以 `"SYNTH FAILED"` 与
   `"SYSTEM BUILD DONE"` 两个**字面量本身**一定在日志里。第一轮看门狗因此在综合进行中就误报
   `BUILD_FAILED_ABORT`；而同一行代码里的成功判据如果不锚行首，会在构建开始后 30 秒就以为跑完、
   拿上一版 bit 去过门禁刷板 —— **假绿比假红危险**。修法：所有标记一律 `^` 锚行首。
2. **台架日志里的中文判据行本来就是坏的**（`skill/bench_verilog_subset.md` 第 9 类）：
   `od -c` 实测，字符串字面量一旦**赋给定宽向量或作为 task 实参**，每个字节的 bit7 被清 ⇒
   PASS 行的中文全成乱码，而直接 `$display` 的字面量完好；定宽字段超长时**丢的是头部**。
   这条影响所有用 `line(tag, ok, txt)` 写法的台架（也就是几乎全部），
   今后判据标签只写 ASCII、解释文字放在直接 `$display` 的字面量里。


### 5）一次编译期反复踩的坑（四类 SV 写法）

`xvlog` 按 Verilog-2001 编，一天之内我撞了四次：`real'()` / `int'()` / `8'()`、
`fork…join_any/disable fork`、**无参 function**、以及 `task` 参数只给第一个写类型
（其余默认 **1 bit** ⇒ `to=13` 变成 1 ⇒ 台架零样本"全过"，靠"覆盖面"那条判据才当场暴露）。
全部整理进 `skill/bench_verilog_subset.md`。

### 6）这一轮的账与下一步

- 采纳判据：门禁 14 项 + 全量 L1 + 板级机器复验 + 一次眼睛（第 22 行）。
- 4b 剩下的三件（几何统一、`split_ctrl` 上顶层、`split/rot/osd` 三个动词）设计已经定稿到
  "可以直接写代码"的程度，写在 `ISSUES #62` 的追加段里，其中两处修正值得单独说：
  **不需要把 `H_ACTIVE` 改成 1024**（今天右窗本来就是一行 512 个有效拍 ⇒ 改成"只在偶数列进链"即可等价），
  以及两个抽头都从同一个第 5 级像素长出来 ⇒ 只要 `skid(15) + 4 行环`，**不需要第二个读口**。
- 双线性（r60）排在 4b 之后；它三次红过时序这件事仍然要在采纳前当着门禁重验。

### 7）13:35–13:57：板级链跑完，但**第一次是判红的** —— 红的是我的门禁脚本，不是仲裁

刷板后第一次机器复验：`[ARB] 结论：1 条不通过 ⇒ 判红，这一版不能采纳`（V1 基线归 PS，
"静默 3s 内 owner_eth=1 的样本 **0/0**"）—— 可是 `board_verify.sh` 的调用方看到的是
`VERIFY_EXIT=0`。两个缺陷叠在一起（记成 **ISSUES #69**）：

1. 这个脚本**从来没有总判定**（没有计数器、没有 `RESULT` 行、结尾恒 `exit 0`）；
2. 每一步都写成 `node … | tail -25 | tee -a $LOG` ⇒ `$?` 是管道最后一个 `tee` 的，
   `arb_handover_test.mjs` 自己 `process.exit(1)` 那条红就在这里被吞掉。

V1 那条红本身也值得记：它判"推流前 3 秒的静默里 `owner_eth` 必须为 0，**而且窗口里要有样本**"。
脚本先起采样会话、紧接着 `const t0 = Date.now()` ⇒ PRE 那 3 秒整个花在 xsdb 连 hw_server +
第一次 `mrd` 的爬坡上；当时机器上同时跑着综合与一台 xsim，爬坡被拖到 4.25 s（样本密度也从
r58 的约 100/117 掉到 73/117）。⇒ **判据自己的时间起点和采样链路抢跑**，不是仲裁坏了。
修法不是把 PRE 放大、更不是删掉 `base.n > 0`（那是把判据阉掉），而是**等第一拍样本真的到了再掐 t0**
⇒ 基线段仍被完整覆盖，该红照样红（样本永远不来则由 V0 密度红）。
...
上下文行第一个样本现在落在 `-3s:own0:live0:tb1`（t1 之前 3 秒、owner=0）—— 就是 V1 想看而以前
看不到的那一格。两份 arb json（爬坡慢的那次 + 修后那次）都并排留在 `build/frozen_r59a_seamtap/`，
"改判据之前为什么红"这件事因此也有凭据。L1 在台架改动之后**重跑**：`SIM DONE pass=64 fail=0`。

### 8）r59a 的账（数字都写在 `build/frozen_r59a_seamtap/MANIFEST.md`）

WNS **+0.262**（r58 是 +0.321 —— skid 被 `SB = MIX_D+1 = 21` 撑长一级选择，**这是代价，不写成优化**）、
WHS **+0.046**（r58 +0.028）、失败 setup/hold 端点 0/0、BRAM 92 tile（65.71 %）、
LUT 11143（20.95 %）、Dynamic 2.184 W、methodology CRITICAL 0、`cdc.rpt` Critical 3 行（基线 4）、
端口宽度警告 0、多驱动 0、顶层接线 81 instance / 548 位宽比较 / violations 0；
门禁 14 项 `GATES: ALL PASS`、L1 64/64、板级仲裁 8/8 + 命令电池 59/59 + 跑完回到初态。
欠的仍然是那三件眼睛判据（`board/README.md` 第 22 行为首）与 C1/C2 那把尺子。

---

## §44 r59b 前置的两件小事 + 一个真功能（2026-09-25 14:20–14:40）

### 1）把 r59a 那句"标签与内容同列"往机器判据推了一步，然后发现它推不动（#68 追加二）

尺子换到正确的级数（列标签**问名字要** `u_split.x_sel`、只在一行中段采样避开 stage-11 行标签的跳行），
量到的不是"差几列"而是 `tap_raw` 六成多格子是 **X**，而且 X 来自更上游的 `fb_out`。
同时上一轮我那句"探针证明拷贝是好的"**作废** —— `dut.aw_wr_en` 是 AXI 写通道的 enable，
不是显示帧缓存 `u_fb` 的写口，所以 115200 那个数说明不了被显示的那一颗被写满。
⇒ 台架新增三条会真的红的判据/规矩：`C0e`（直方图超量程 ⊆ 不符集合）、`C1a`（行中段样本数下限）、
以及凡"逐格相等"的判据必须显式带"X 占比 <10 %"（第一版 `C1c` 就在整屏 X 上假绿过：
`dsub(X,..) != 0` 是 X ⇒ 计数不涨 ⇒ "零个错"）。C1b/C1c/C1d 降为报数并写明三步前置条件。
"标签与内容同列"今天唯一的凭据仍是板级眼睛（`board/README.md` 第 22 行）。

### 2）两个工具窄面（都进 skill）：xsim 的 `$display` 不认 `%+d`；非 ASCII 文本一经定宽向量就被砍掉 bit7

前者会把整行参数对位全带歪（读起来像对的，更难发现），全仓台架改 `%0d`；
后者用 `od -c` 量出来了（`甲`=E7 94 AD 直打正常、经 task 实参变 EF BF…），
⇒ 判据标签只写 ASCII，中文解释留在直接 `$display` 里。

### 3）真功能：SD 卡上 9 段可选（`sd files` / `sd file <n>`），V8-9

以前"想看第 3 段"只能人肉算 `frame 1536`，而段表（META.TXT 的 FILEn）本来就在固件里。
板上实测：`[SD] 9 file(s), 4398 frame(s) total` + 每段 `#n 名字 帧数 first=全局帧号`；
`sd file 1` → `[SD] file #1 VIDEO001.BIN (512 frame(s)) first=512`；
`sd file 99` → `[SD] file 只认 0..8（不认的写法不改任何状态…）`，**拒绝且不改状态**（#67 那条规矩）。
判据：电池第 60..63 条（`--align` 先证明命令表与判据表同序：63/63），全 63 条 PASS 且回到初态；
`RESULT board_verify` 那套总判定本轮已修好（#69），所以绿是真的绿。
PL 三件套没动（bit `f2ebcd4a`、xsa `d6544177` 与 r59a 同一份），elf 换新 `ff37ebe6`，
门禁重跑 `build/gates_r59b_sd.txt` = `GATES: ALL PASS`。

### 4）本轮的账（未做的）

r59b 几何那一半（整屏单地址流 + 效果链吃满 + 输出侧 ×2 展开）与 `rot`/`osd` 两个动词（位图已按 #70 定稿）
仍然排队；`Split:50 %(Auto)` 那一格与"分割线 0~100 % 可调"这条主张继续不写。未推 GitHub。

---

## §45 收工账本（2026-09-25 15:45）：三项落地、一项被门禁挡下、两项排队

**板上现在跑的**：`system.bit f2ebcd4a` / `system.xsa d6544177` / `ps_app.elf ff37ebe6`
= r59a + V8-9 那一套；`RESULT board_verify PASS（判红的步骤：0）`、门禁 14 项 ALL PASS、L1 64/64。
冻结件与全部凭据在 `build/frozen_r59a_seamtap/`。工作树干净（`git status` 无修改项）。

**落地的三件**
1. **r59a** —— 缝的"选哪一路"改站到与两个像素抽头同一级（`MIX_D = 3+1+1+u_pipe.LATENCY` 推出来，
   不再抄字面量 11）+ 蓝线开关 `marker` + 4 行环形原图延迟线。代价写清楚：WNS 0.321 → 0.262。
2. **V8-9** —— `sd files` / `sd file <n>`：卡上 9 段可选，越界明确拒绝且不改状态（板上实测回包在
   `board/uart_script_capture.txt`）。
3. **#69** —— `board_verify.sh` 以前**没有总判定**、每步退出码又被管道吃掉 ⇒ 一条 arb 判红曾被吞成绿；
   现在每步既看 `$?` 也看输出里那句绿色总结，末尾给 `RESULT board_verify PASS/FAIL nred=N`。

**被门禁挡下的一件（红留档，不删）**：**#71** = V8-10（`rot`/`osd` 上硬件）整套做完、L1 64/64 也过、
PL 构建 #44 收敛（WNS +0.702、BRAM 92/65.71 %、Dynamic 2.192 W），但 `cdc.rpt` 的
**unsafe 端点 1→5** 判红 —— 根因是我把那条已批准的 ASYNC_REG 链从 24 位加宽到 34 位，
而 unsafe 是**按配对数端点**的；`#70 追加` 里"配对不新增 ⇒ 基线不用动"那句只对了前半句。
实现整体保留在提交 `efbd2b9`，工作树回退；三条出路写全在 #71（① 这 10 位走仓库现成的 `snap_cross`；
② 挤进 gamma 的 `idx/data` 窗口协议，不加宽任何链；③ 写清理由才允许动基线 —— 最像松尺子，
除非 ①② 都做不起不许走）。

**排队两件，各自的下一刀已经具体到行**
- **#50 r59b 几何统一**：第一刀在 `pl_video_top` 的读口/坐标段 —— 整屏一条地址流（1024 列、同源列喂两次）、
  效果链只在偶数列进链（`de_in = de_d[3] && !x_d[3][0]`，`H_ACTIVE` 保持 512，理由已写进
  `proc_pipeline.v` 文件头那条 ⚠）、两抽头在 `MIX_D` 级混合、输出侧 ×2 展开必须在 OSD 之前。
  做完才允许写"分割线 0~100 % 可调"，今天仍然不写。
- **#54 把 r59a 变成机器可证**：测前把缩放钉成手动 1.00×（`zsel=4 / zman=1`），再把"搬运只在消隐里走"
  的节奏建模出来（现在五成 X 最像单口 BRAM 读写同拍冲突），然后把 `C1b/C1c/C1d` 从报数升成判据。
  在那之前 r59a 的"标签与内容同列"唯一凭据是眼睛：`board/README.md` 第 22 行。

**还给你的三眼**：第 22 行（缝左右 10 列有无暗带）、第 21/21b 行（蓝线该在就在、该没就没）、
`sd file 1` 切段屏上是否真换了内容。未推 GitHub（等你一句话）。

---

## §46 r51（2026-09-25 17:55–18:55）：分割线真的可动了 —— 4b 的两块（单视口 + 逐像素混合）今天合上了

`split_ctrl`（原来只有台架、没有顶层例化的那块）接上顶层，缝位/自动扫/follow/swap/蓝线开关
全部由串口驱动；`split_display` 的 `PANE_W` 参数**删掉**、缝位改成输入 `seam`
（删参数而不是留默认值：这样四份台架 + 两个顶层例化点都被迫表态，不会有一个台架"以为自己在测极端缝位、
其实还钉在 512"）。OSD 第四行 `Split:` 从 `SPLIT_PCT_FIX` 死数改成 `shown_pct` 真值，死数连同它的
推导注释一起删 —— 留着就是 #66 那一族的"两处各说一遍"。

控制位 14 个（`CFG_DATA0[22:13] + [23]auto + [24]follow + [25]swap + [30]marker_off`，位图唯一出处仍是
#70 追加）**走 `snap_cross`**，axi 域每 ~1.3 ms 整拍抄一次再翻 toggle。为什么不走那条"看起来更省"的
路（并进 effect_ctrl 现成的 ASYNC_REG 链）：**#71 就是今天刚撞的墙** —— 加宽那条链会让
`cdc.rpt` 的 unsafe 端点按位长涨，门禁第 11 项直接判红。这一轮一次过。

数字（`build/frozen_r51_seam/`）：门禁 14 项 ALL PASS —— **WNS +0.097 / WHS +0.017（本项目至今最薄）**,
BRAM 96/68.57 %, LUT 12793/24.05 %, 寄存器 9422, Dynamic 2.207 W, cdc Critical 3（基线 4）,
violations 0；L1 64/64；板级仲裁**九条**全过、命令电池 **71 条**全过且回到初态、
`RESULT board_verify PASS（判红的步骤：0）`。
薄的那一档不当好消息写：MANIFEST 里给了两个不动约束的杠杆（`split_eff` 提前一拍寄存；端点乘法折到帧首），
下次改几何前先做这两件，别等它变负再找。

## §46 补：今天被门禁救下来的一次误红，以及把 V1 拆开
18:36 那次 `V1 基线归 PS` 红（基线里 `owner_eth=1`）。当时没有本机推流进程，事后读板 `eth_live=0`、
包数 6 秒不动 ⇒ 那次是**别人在推流**（用户那边的窗口工具）。但 V1 原来一条判两件事：
"外部在推流"与"没有流却还占总线"（后者才是仲裁坏了）。lane30 本来就有独立的 `eth_live` 位，
所以拆成 **V1（前提：静默）+ V1b（owner=1 且 live=0 必须为零）**：拆完 V1 的牙没变钝，
无流却占着照样红，只是不再把环境当硬件报。判据从八条变九条 ⇒ `board_verify.sh` 的绿色标记与
退出码一起跟上（#69 之后它才有真退出码；这次也是它把 `RESULT board_verify FAIL nred=1` 报出来的）。
