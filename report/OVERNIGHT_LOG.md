...
| **R23** | `eb42175`(+注释) | **`534f7760`** | 1855990 B | **+1.002** | **明早默认这一块**：V7.7 功能 + R22 两笔 + `copy_abort` 翻转式脉冲同步（ISSUES #36 结案）。L1 **40/40**。成套冻结 `build/frozen_r18_abort/`；xsa `9dd8d236`、elf `3837c7fc`（固件本轮未动）。注意：**综合之后仓库里又只加了注释**，网表与源码一致这句话要说成"除注释外一致" |
