...
交回 **406 / 434 / 210 ms**，七条判据同样全绿 ⇒ 对外报 **0.2–0.45 s 区间 + 分辨率**，
...
### 明早清单（覆盖 §22；2026-09-24 05:3x 最后一次更新，按"卡住演示的程度"排）

0. **（先做这个）SD 卡的定点坏点：要么验证要么绕开。** 播放到第 3584 帧
   （第 8 个文件 VIDEO007.BIN 的第一帧）**必然**读失败 ⇒ 演示循环跑到第 2 分钟就会停。
   两条路，选一条：
   · **验证**：卡拔到 PC 上，逐文件比 md5 与长度（同源件在 `D:\Xilinx\sd_stage_r29`），
     再看 VIDEO007.BIN 的首簇 / FAT 项是否越界；卡对不上 ⇒ 重拷就完事（`src/host/make_sd_video.mjs`）。
   · **绕开**：把 `META.TXT` 的 FILE 行改成 8 个文件（去掉坏的那个），回放就变成 3886 帧的干净循环；
     代价是片长短一截，**并且这是"绕过"不是"修好"**，文档里会照实写。
   不用断电：`ps_jtag_boot → program_pl → ps_app_reload` 一遍就能把控制器救回来（今晚两次都成）。
...
3. **（等你点头）一笔没对上的账**：`cdc.rpt` 里 `eth_rxc → clk_fpga_0` 从 #23 的 Critical（272 端点）
   ...
   要不要花一轮查？不查也行，但口径继续按"不记成改进"（§10 U14）。
