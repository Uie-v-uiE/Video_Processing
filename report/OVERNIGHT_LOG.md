...
### R31-C · 构建与状态：**这一版不冻结**（我自己预测错了，数据摆在这儿）

| 项 | r23 冻结件（上一条 KU5P 成套） | r31 第一版（乘法进 DSP） | r31 第二版（移位相加） |
|---|---|---|---|
| WNS / WHS (ns) | +1.916 / +0.010 | +0.647 / +0.013 | **+0.192** / +0.013 |
| 失败端点 / 总端点 | 0 / 11668 | 0 / 12982 | 0 / 12982 |
| CLB LUT / FF | 3021 / 2799 | 3681 / 3258 | 3693 / 3258 |
| BRAM tile / DSP | 72 / 0 | 72 / **4** | 72 / **0** |
| 布线 / methodology CRIT / cdc 行 | 0 / 0 / 1 | 0 / 0 / 1 | 0 / 0 / 1 |
| 最差路径 | — | `u_rx_cmd/p_data_reg[5]/C → u_cmd/cmds_ok_reg[5]/CE`，20 级含 DSP | 同一条链，14 级（`→ u_cmd/cmd_clr_reg/D`） |

判据与结论：两版都"0 违例、约束全满足"，但 **WNS 从 +1.9 掉到 +0.2 不是噪声**（本设计里我量过的
轮次噪声是 ±0.2 一级，而这里是 −1.7）。我原本以为掉的那 1.27 ns 是那条十进制折叠被塞进 DSP48 造成的 ——
换成移位相加之后 DSP 归零、级数 20→14，**WNS 反而更低** ⇒ 假设被自己的数据推翻：
真正的瓶颈是整条"宽比较"匹配锥（`head == "SPD"` 的 24 bit 比较 + 三个 `is_digit` + 三向 mux
全挤在 125 MHz 的一拍里），跟乘不乘没关系。

⇒ 修法换成**逐字节状态机**（每拍只与刚进来的那个字节比：8 bit 比较器 × 3 + 三个标志位，
包尾只看标志，不再在包尾做组合大减法）；改完重跑 `tb_v80_ku5p_cmd`（35 条不许变绿就算白改）
再建一次，WNS 回不到 +1.5 以上就把这一节的实测原样留在提交物里，不挑好看的报。
两份报告都留档：`ku5p/build/ku5p_setup_r31_dsp.rpt`（第一版）与本次的 `ku5p_setup.rpt`（第二版）；
bit `9ca5a6c2` / `7b96536f` 都不进冻结件。
