...
| KEY1 **长按 0.6 s** | 片源模式走一格。**OSD 第 6 行 `SRC=` 跟着变，但它画的是屏幕上真的那一路**：`CARD` / `PS` / `ETH`；末尾出现 `*` 表示此刻是手动锁住的（没有 `*` 就是自动）。显示 ETH 时按一次 → `SRC=PS*` |
...
> （AUTO/锁ETH/锁PS/锁图卡），于是"显示 ETH 时写着 AUTO"、"CARD 和 PS 看着反了"都会发生 ——
...
```
src 0        图卡（会动的渐变底 + 彗星，自带帧号格）
src 1        DDR（网线有流时是网络画面）
src 2        DDR 并立刻开始播 SD 卡序列
stat         回读：en / thr / src / zoom / bilin / sd / frames / playing / sel
```
> `src` 只切 PS 手里那 1 bit `src_sel`。**"独占哪一路"目前是按键的事**（模式覆盖位排在下一版），
> AUTO 下屏幕归谁仍由仲裁决定：ETH 有流 → ETH；停流几百毫秒内自动交回 PS。
