...
9. **三件等用户表态、不该由我替他决定的事**（详表在 `report/PLAN_V8_SPEC.md` §3 末尾那段）：
   4b 的几何语义（决定"分割线 0~100 % 可调"这条 spec 硬要求做不做）、
   步 7 的"回落原因要不要上屏"（现在只在 lane30 机器可读）、
   步 9 双线性要不要再开一整晚（它三次红过时序，是唯一能消掉 8① 的东西）。
