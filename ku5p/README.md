...
**已跑出来的构建结果**（`ku5p/build/ku5p_*.rpt`；下表是 **R31c**（命令通道 + 逐字节匹配状态机）
那一次，2026-09-23 23:1x；括号里是上一版成套件 **r23**（只有遥测 + 发送仲裁器）：

| 项 | R31c | 说明 |
|----|----|------|
| WNS / WHS | **+1.985 / +0.011 ns**（r23 是 +1.916 / +0.010） | 时钟只有一根：`create_clock -period 8.000`（125 MHz，PHY 恢复时钟）；`All user specified timing constraints are met` |
| 失败端点 / 总端点 | 0 / **12906**（r23 是 11668） | 命令通道 + 第二个端口过滤器多了 ~1240 个端点 |
| CLB LUT / FF | 3612（1.67%）/ 3220（0.74%） | 只装了入口 + 收发那一半，没有显示通路 |
| Block RAM | **72 tile（15.0% of 480）** | 与 r23 **一模一样** ⇒ 新增逻辑没有要 RAM；推导见 §5 |
| URAM / DSP | 0 / **0** | 这块板有 64 块 UltraRAM，本设计一块没用（§9 第 2 条的题）；DSP 也刻意保持 0（见下面那条时序教训） |
| 布线 | 全布通，**0 条布线错误** | `write_bitstream` 出 15.4 MB 的 `ku5p_eth.bit`（md5 `5ce3578c`） |
...
> **一条时序教训（R31 当场踩的，数据都在）**：命令解析第一版把整包收进移位寄存器、
> 在包尾那一拍做"24 bit 前缀比较 + 判数字 + 十进制折叠"，WNS 从 +1.916 掉到 **+0.647**，
> 报告里最差路径出现 `DSP_ALU/DSP_MULTIPLIER`（综合把 `v*100` 塞进了 DSP48）；
> 我以为是乘法器的事，换成移位相加后 DSP 归零、级数 20→14，**WNS 反而掉到 +0.192**
> ⇒ 假设被自己的数据推翻：真瓶颈是"包尾那条宽比较组合锥"。
> 改成**逐字节状态机**（每拍只看刚进来的那个字节）之后 WNS = **+1.985**，比基线还好一点，
> 最差路径也回到了厂商 ICMP 发送 FSM（不再是我加的逻辑）。
> 三份报告都留着：`ku5p_setup_r31_dsp.rpt`、`ku5p_setup_r31_shift.rpt`、`ku5p_setup.rpt`。
> 判据：`sim/tb_v80_ku5p_cmd.v` 35 条在三次改写里**一个字没改**、始终全绿 —— 改的是实现，不是判据。
