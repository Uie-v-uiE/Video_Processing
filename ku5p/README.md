...
**还没解释的**：同一份 `frame_buffer_w64` 在 7 系列报 80 tile、在 UltraScale+ 报 72 tile，
而按信息量下界只要 7~8 tile。值得单独查一次（`ram_style`、64 bit 宽度下的深度切分、
或者两个数组各占多少），别当成" UltraScale 更省"直接说出去。
