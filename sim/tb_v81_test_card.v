...
module tb_v81_test_card;
    ...
    initial begin
        ...
        // ---- T16 模式格雷码环：验 `pl_video_top` 里手推的那条下一状态式 ----
        // 顶层写的是 `mode <= {mode[0], ~mode[1]}`（四状态：自动 00 → 锁ETH 01 → 锁PS 11 →
        // 锁图卡 10 → 自动 00）。这条式子是手推的，所以要有独立证据；这里验的是**式子本身**
        // （四步依次落到四个模式、且每步只动一位 = 格雷码的定义），
        // 它**不**验顶层有没有把它接对 —— 那一半是板级长按判据（`board/README.md`）。
        // 四个编码在这里**重抄一遍**而不是从 RTL 引用，是为了让"改了一边忘了另一边"能红。
        begin : gray_ring
            reg [1:0] m, m0, m1, m2, m3;
            m = 2'd0;                             // 起点 = 自动
            m0 = {m[0], ~m[1]};  m1 = {m0[0], ~m0[1]};
            m2 = {m1[0], ~m1[1]}; m3 = {m2[0], ~m2[1]};
            expect("T16 四步环依次落在 锁ETH/锁PS/锁图卡/自动",
                   m0 == 2'd1 && m1 == 2'd3 && m2 == 2'd2 && m3 == 2'd0);
            expect("T16b 每一步只有一位变化（格雷码成立才允许打拍跨域）",
                   (m ^ m0) == 2'd01 && (m0 ^ m1) == 2'd10 &&
                   (m1 ^ m2) == 2'd01 && (m2 ^ m3) == 2'd10);
            expect("T16c 四步回到起点且四个状态互不相同（没有进不去的模式）",
                   m3 == m && (m != m0) && (m != m1) && (m != m2) &&
                   (m0 != m1) && (m0 != m2) && (m1 != m2));
        end
