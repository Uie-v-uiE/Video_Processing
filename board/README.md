...
| 9 | **（#31/#32 待确认）** `KEY1` 长按 1.2 s（**r46 起阈值改 0.6 s，且短按改松手才发** —— 见第 13–15 行） | 每按一次四态轮转一格：自动 → ETH → SD → TEST → 回自动（屏上 `Src:` 印的就是后三个词，2026-09-25 定稿）；停在 **TEST** 时那张卡必须在动。已知代价：长按会顺带 +1° 旋转（同一根键的短按语义），这条不算 FAIL。机器侧已证 lane30 能读回模式与两位归属（`node src/host/health_read.mjs`），缺的只是"屏上真的换了来源" |
...
另外：中线本身仍有一根**故意画的 2 像素蓝线**（`split_display.v`，`marker` 入口现在能关，默认仍开），那是标记不是缺陷 ⇒ 关掉它之后屏上必须**一列蓝都没有**（`tb_v97` S7a 钉住）。
⚠ **r59a 之后这一条的期望要重新看**：以前混色级用第 11 级的列标签去选左右两路，而像素内容站在第 20 级（差 9 列 = 一条近黑的竖带，ISSUES #68，也就是"碰到分割线周围出现颜色条"的第二个成分）。r59a 把选择坐标换成与内容同级的 `x_sel` ⇒ **请专门看缝的左右两侧 10 列以内还有没有暗带/错色带**；如果还有，请说它是"一条黑"还是"一条蓝"还是"内容错开一格"（三种修法不同：分别对应 #68 未修净、`marker` 该关、#54/#56-2 那一族）。这一条仍然只有眼睛能判完：`tb_v92/tb_v97` 量的是模块，顶层的接线正确性靠 `tb_v98_top_seam` 的 C-tap（坐标级）+ 你这一次的眼睛（观感级） |
| 13 | **（r46 起，r48 改标签语义）长按一次就要看见结果**：推流中长按 KEY1 一次 | OSD **第一行**的 `Src:` 那一格画的是屏幕上真的那一路（只有三个词：`Src:ETH` / `Src:SD` / `Src:TEST`，V8-5 四行格式），末尾一个 `*` = "这是手动锁住的"。所以推流中按一次：`Src:ETH` → `Src:SD*`，画面同时从网络切到 SD。**旧版把"意愿"（AUTO/ETH/SD/TEST 四态）当事实画，才会出现"显示 ETH 时写着 AUTO""两个词看着反了"** —— 歧义已从结构上去掉：机器判据故意造了"仲裁以为占着屏、mux 却选了图卡"这个状态，标签必须跟着屏幕走（`tb_osd_lines`）|
...
| 15 | **（r46 起，r48 改过内容，r58 换词）字模完整性** | `Src:ETH`/`Src:SD`/`Src:TEST` 与第二~四行的小写（i p e r c a m n o s t u x y）里不能有字是空白或缺一笔：这次真正用到的新字模是 **U、H 与 `*`**（`*` = 手动锁住标记；`U` 因为不再显示 `AUTO` 而退出这一行，但字模留着给 V8-5 的四行格式）。`tb_osd_lines` 逐像素对位图、`tb_v794_osd_glyph` 另有一份独立的码点→字形金表 |
...
| 20 | **（图卡 v2，任务 #33）`SRC0` 切到图卡，盯背景与那颗移动的头**：看 5 秒 | 机器侧已经钉过的不用再看：`tb_v83_card_render` 量了背景**单调**、球与光晕的位置/半径、帧号区。**只剩观感**：深蓝背景是不是"一条渐变"而不是分层横带（历史上这里踩过：用 `y[7:6]` 这种分段位做渐变，出来是一层一层的横带，台架量得出台架也不想看）、那颗头拖的尾巴顺不顺、有没有哪一帧明显跳一下。看到"横带/断层/尾巴闪烁"请说是哪一列到哪一列 |
| 21 | **（r58，用户坐在屏幕前报上来的三条）**三件事一次看:① 第一行 `Src:` 与第四行 `Split:` 的字形；② 第二行 `Pipe:… Th:… Gamma:…` 的**右端**;③ 串口 `src 2` → `src 1` → `src auto` 各看一眼，然后在**任何一种钉住态**下长按 KEY1 一次 | ① 用户原话"`Src` 写得像 `Sro`、`Pipe` 的 `p` 和 `e` 一样高"——根因是字模：`c` 的右列在中间两行多点了两个点（就成了 `o`）、`p` 的左列在第二行断开（就成了"o 加个尾巴"）。两个都重画了，金表**在台架里独立手写**（`tb_osd_lines` T14），所以这次不是"改完自己说自己对"。**要看的是不是真的像 `c` 和 `p` 了**（字形这种事机器量位图、人读观感）。② 现在必须在分割线**这一侧**（字格 18 px、起点 16 px，28 格就到 520 px > 511 就是压线；收成单空格后 26 格 = 484 px，`tb_osd_lines` T13 逐行量）。③ 这条是"命令救不出来"那一账的还债：以前 `src 0/1/2` 只写 1 bit，模式环只有按键一个入口 ⇒ 长按几次才切得动。**期望**：`src 0` ⇒ 屏上是片内自绘图卡且 `Src:TEST*`、`src 2`（且卡里放得动）⇒ `Src:SD*`、`src 1` 且正在推流 ⇒ `Src:ETH*`、`src auto` ⇒ `*` 消失且回到按键环；**钉住态下长按一次只交还控制权**（画面回到 AUTO 该给的那一路，不该又钉到另一态）。串口同时能看到 `[SRC] 已钉住 …（mode=N）`，`STAT` 末尾的 `mode=` 就是 PL 收到的那两位。<br>词表这件事**当场了结**：用户随后一句"就叫 ETH SD TEST"⇒ 屏上三个词、串口消息、固件里的档名（`M_ETH/M_SD/M_TEST`）、`health_read` 的译码名、`arb_handover_test` 的名字表**一次统一**，**编号一个没动**（AUTO=0、ETH=1、TEST=2、SD=3，格雷码性质与 #52/#55 那两条教训保持）。旧文档里凡写"锁 PS / 锁图卡 / CARD / `Src:PS`"的地方指的就是现在的 SD / TEST —— 那是历史记录，不改写（改了就没法看出当时错在哪）。`tb_osd_lines` T3 钉三个词各归哪一态、T13 用**最长的那个词**量行宽（换成 TEST 之后如果还拿 SD 当"最宽激励"，这条判据就没牙了 —— 台架注释里写着） |
| 21b | **（r58 附带）`pipe show` 与屏上那五位**：串口 `pipe 000100000` → `pipe show` → `pipe 00001100` | 屏上 `Pipe:` 那五位**不是**命令里那串 0/1（每格是"这一级选了第几个算法"，所以会出现 `2`，那是合法的：见 `report/COMMANDS.md` §1）。`pipe show` 必须按名字念出当前生效项（`[PIPE] sel=008 生效: sharpen`）；最后那条**必须被拒**并说明只收 5 或 9 位，而且屏上**不许有任何变化**（拒了还写寄存器等于没拒） |

| 22 | **（r59a，ISSUES #68）只看一件事：缝左右各 10 列以内，有没有一条暗带 / 错色带**。做法：`src 0`（图卡，自带 32 像素网格）+ 右窗随便开一级（`pipe 000010000` = Sobel 最明显），眼睛贴到中线两侧各约 10 个像素的范围上扫一遍；（r59a 里 `marker` 入口已经存在但顶层仍钉死为 1 = 画线，串口**还没有**关它的命令 —— 那要等 r59b 的 `split ...`，所以现在只能看、不能关。） | 修之前的机理：混色级用第 11 级的列标签去选"这一格取原图还是取处理图"，而两个像素抽头都站在第 20 级（`3 + 1 + 1 + u_pipe.LATENCY`）⇒ 有 9 列"标签说左窗、内容其实是右窗那一路（被强制清 0）"⇒ 选中 0 ⇒ 一条约 9 像素近黑的竖带。**期望（r59a 之后）**：那条暗带**消失**，中线上只剩故意画的 2 px 蓝线（细、且两侧内容连续）。<br>**三种可能的回报，请分类说**：① 还是一条**黑的/暗的**竖带 ⇒ #68 没修净（标签级数还差着），修法是把 `MIX_D` 的算式补对；② 是一条**蓝的**竖线 ⇒ 那是特性，`marker` 一关就没有（顺便就验了关线）；③ 两侧**内容错开一列/一行** ⇒ 那是 #54 / #56-2 那一族（行缓存边界守卫），不是 #68。<br>为什么这条必须眼睛看：能判"标签与内容同列"的两条路里，仿真那条要先把 DDR→帧缓存的搬运建模对（`tb_v98_top_seam` 里 `NOTE C1/C2` 记的就是这个缺口），剩下的直接凭据就是屏上这一眼 |
| 23 | **（r59b-1，#73/#44）整幅铺满 + 旋转缩放左右一致**。做法：`src 0`（图卡，自带 32 像素网格与帧号）→ 长按 KEY1 进旋转、或用按键调角度，再看 `zoom 0.5` / `zoom 1.0` / `zoom 2.0`；然后把右窗开一级 Sobel（`pipe 000010000`），眼睛扫中线左右 10 列 | 这一轮把"两屏各画一整幅"改成"**一幅铺满 1024**（每个源列占 2 个显示列）"，所以观感有三件事同时变了，请分开报：**① 画面比原来大了一倍**（这是可动缝的前提，不是 bug）；**② 旋转/缩放现在左右两半一起动**（以前只有右半动，所以你会看到"左半不动右半转"，那已经是旧行为）；**③ 横方向的模糊/边缘比旧版略宽一点** —— 这是 #73 里写死的代价（链子吃满 1024 列 ⇒ 3×3 覆盖 1.5 个源列），不是错位。**仍然要看的**：中线两侧内容必须**连续**（缝还钉在 512 = 屏幕正中，`split` 命令要到 #51 才有）；转 90°/180° 时左右两半的内容应当是**同一幅转出来的**，不该一半转一半不转。**报法**：③ 之外若出现"左右两半错开一列/一行"⇒ 那是 #54/#68 那一族（原图抽头的 4 行行环或 skid 级数），直接说开几列/几行；若画面比原来**小**了或者只铺满一半 ⇒ 是 `cx = x>>1` 这一级的视口范围错了 |
