...
| 13 | **（r46 起）长按一次就要看见结果**：推流中长按 KEY1 一次 | OSD 第 6 行的 `SRC=` 必须**当场**变成 `PS`（因为当前显示的是 ETH，AUTO 那一格直接跳到锁 PS，不再走"锁 ETH"这种看不出变化的空动作），并且画面从网络切到 SD；**顺带旋转 1° 的现象应当消失** |
| 14 | **（r46 起）按住过程中的反馈** | 按住约 0.2 s 后 LED1 亮、到 0.6 s 生效时 LED1 翻转并保持；松手后 LED1 不再随短按动。这一条是给"有时长按没反应"的判据：有反馈时用户不需要重按 |
| 15 | **（r46 起）新字模 U / H** | OSD 上 `SRC=AUTO` 与 `SRC=ETH` 必须是完整字母（以前字库里没有 U 和 H，缺字模的表现是**静默变成空格**，看着像 `SRC=A TO`）；`tb_osd_lines` 已逐像素对过，这条是确认硬件上也是同一份字模 |
