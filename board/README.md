...
**r54 更新**：lane23 已经上板 ⇒ ①② 两件事现在有机器对照（`health_read.mjs` 的 `23` 行：
`zman=1`、`inv=期望值`、屏上档 `zcode==zsel` 三条同时成立才算绿），
所以眼睛这一半**只剩 ③④**（有没有黑边、呼吸继续走还是弹回）——这两条是"画面上真的那样吗"，机器看不见。 |
| 19 | **（r54 起）片上温度**：串口 `temp` → `temp th 0` → `temp` → `temp th 85` → 再用指腹捂住芯片几秒后 `temp` | 前三步已经在串口电池里机器判过了（`over` 必须跟着阈值 0/200 亮灭、`sane` 必须是 1），**只有最后一步是眼睛/手的**：温上去之后 `degC=` 必须**爬**（几分度即可，别指望秒级），凉回去再降 —— 一直显示同一个数就是只读到了复位值。开机那行必须出现 `[TEMP] PS-XADC @f8007100 ok`；若出现 `[TEMP!]` 说明 raw 不像一次真实转换（−273.15 或 230.8 °C 一类），此时的读数一律不许写进报告。注意这条走的是 **PS 侧** XADC，PL 里没加任何 IP，"零厂商 IP"的主张不受影响 |
