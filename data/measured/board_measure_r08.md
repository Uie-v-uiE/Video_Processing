...
## 拔线实验（20:42:03–20:43:54，每秒一次采样，`--test move` 15 fps）

采样用一条 xsdb 会话连读 110 次 lane2/lane8/lane31（避免每次重开会话）。
**这张表推翻了 R08 设计时的一个假设，也推翻了上面 §5「拔网线会让 PHY 停供 RXC」的说法。**

| 时刻 | stall_ms | pkts | Δpkts/s | lane31 心跳 |
|------|----------|------|---------|-------------|
| 20:42:30–35 | 0 | 59008→76025 | **3315 / 3536 交替** | alive |
| 20:42:36（拔线） | 566 | 77572 | 1547（该秒被切断） | alive |
| 20:42:38 … 20:42:52 | 831 → **1117** | 77572（**冻结**） | **0** | **仍然 alive** |
| 20:42:53（插回） | 0 | 79341 | 1769 | alive |
| 20:42:54 之后 | 0 | 82656→ | 3315/3536 | alive |

### 三条结论

1. **看门狗和计数完全正确**：断流期间 `pkts` 一动不动，插回后立刻恢复；
   推流时 `Δpkts/s = 3315 = 15 fps × 221 包/帧`，与上位机标称**逐位吻合**
   （个别秒显示 3536 = 16 帧，是 1 秒采样边界截断，不是速率漂移）。
   插回后 `stall_ms` 立即归零，**全程没有重启板子或重下 bit** —— 自动恢复成立。
2. **`hb_gone` 没有触发，因为 RTL8211F 在链路断开时并没有停掉 RXC。**
   证据不只是"lane31 一直是 0"，还有一条更硬的：断流那 13 秒里 `stall_ms` 只走了
   831→1117，即 **+20.5 计数/秒**，而不是 1000/秒。`stall_ms` 的唯一时基是
   `ms_tick = (ms_div == 124999)`，所以 **eth_rxc 当时只有约 125 MHz / 48.6 ≈ 2.5 MHz**。
   ⇒ 时钟"在，但被拉慢约 50 倍"，心跳因此永远不会超时。
3. **所以 `stall_ms` 在断链期间不是秒表，只是"越来越久"的序指标。**
   这一点必须写进口径：屏上 `STALL` 在链路正常时是真实 ms（前面 15 fps 那组已验证
   `gap_last` 52–74 ms 对应 66.7 ms 标称），断链后它按 ~1/48 的速度爬。
   真正能一眼看出"没流了"的判据是 `flags.bit3 流活着` 与 `pkts` 停止增长，
   而不是心跳位。

### 由这条实测得到的修法（下一步）

`snap_cross` 现在只测"心跳有没有停"，应该同时测"**心跳来得快不快**"：
正常边沿间隔 1 ms，退化时约 50 ms，所以加一个 `hb_slow` —— 边沿到来时如果
`to_cnt` 还远高于门限（说明间隔 < SLOW_MS），就判定时基被拉慢 ⇒ 链路已断。
这样"拔网线"在板上就有一个真正会亮的位，OSD 也可以在 `hb_gone || hb_slow` 时
把 `STALL` 钉成 9999（含义从"多少毫秒没帧"变成"至少 SLOW_MS 没帧 / 没有链路"）。
