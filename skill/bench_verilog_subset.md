...
## 第 11 类（同日 14:50）：`integer` 与无符号 net 比较时，-1 哨兵会变成"永远不成立"

```verilog
integer xhi = -1;  wire [11:0] c1_col;
if (c1_col > xhi) xhi = c1_col;      // 实测：xhi 永远停在 -1
```
Verilog 的比较把两个操作数**按无符号**统一位宽：`-1` 被换算成一个巨大无符号值 ⇒ 条件永不成立，
`log` 里就出现 `落在列 25..-1` 这种"看着像初始化没跑"的假象。
两种写法安全：`if ({20'd0, c1_col} > xhi)`（把无符号那侧显式扩到 32 位再比），
或者别拿 -1 当哨兵（用 `integer have = 0;` 加一个"第一次"分支）。
同一族的还有 `real→integer` 自带四舍五入（第 1 类）与 `task` 参数默认 1 bit（第 8 类）——
共同点是**不报错、只给一个看起来像初值的数**。

## 第 12 类（同日 15:01，**宿主脚本**侧）：用 heredoc 里的 Python 生成 C/Verilog 时，转义会被 Python 先吃一遍

`python - <<'PY' ... "+RL+" ...` 这种写法里，如果模板用的是**普通字符串**，`'
'` 就已经是真换行，落到 C 源码里就是"字符串字面量中间断行"，gcc 报
`missing terminating " character`；而报错位置在下一行，于是很容易往错的方向查
（本轮我一度怀疑是宏名冲突，绕了一整轮）。
三条规矩：① 生成含转义的源码优先用编辑工具直写；② 一定要脚本生成时用 `chr(92)` 拼反斜杠，
并且**写完立刻编译/仿真一次**，不要把三处改动攒到一起再验；③ 别把 `"+RL+"` 这类占位符留在生成结果里
（它会以"未声明标识符"的面目出现），要留就在生成后立刻全文搜索确认没有残留。
