...
module tap_sched #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // 左窗地址与车道在**同一个沿**采进快域（s4 结束沿 = s0 起始沿，同一个物理沿：
            // 慢域那一整拍都稳定，这里只是把直线落一拍，好让 s0 的 mux 输入全是本域触发器）。
            // 必须成对：分开采会"字对车道错"，板上现象是整列颜色对、左右偏一格，且不会报错
            // （台架 auxbad=59/61 抓到过一次：字来自上一拍、车道来自本拍）。
            if (s4) begin
                aux_q_addr <= aux_word;
                aux_lane_x <= aux_lane;
            end
